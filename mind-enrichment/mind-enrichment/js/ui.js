...
async function fetchMyEducatorProfile(){
  if(myEducatorProfile) return myEducatorProfile;
  const { data } = await sb.from('educators')
    .select('full_name, headline, bio, years_experience, verification_status')
    .eq('user_id', currentUser.id).single();
  myEducatorProfile = data;
  return data;
}

async function showEduDashPanel(panel){
  ...
  if(panel==='profile'){
    ...
    const p = await fetchMyEducatorProfile();
    ...
    el.innerHTML = '<h3>My profile</h3><p class="panel-sub">This is what parents see on the marketplace.</p>'+
      ...
      '</div>'+
      '<p style="font-size:13.5px;color:var(--text-secondary);margin-bottom:16px;">'+(p.bio||'No bio yet.')+' &middot; '+(p.years_experience||0)+' yrs experience</p>'+
    ...
    const { data: quals } = await sb.from('qualifications').select('title, institution, year_obtained, verification_status').eq('educator_id', currentUser.id);
